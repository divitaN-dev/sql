...
SELECT market_date, total_sales, 'Best Day' AS sales_day_type
FROM ranked_sales
WHERE best_day_rank = 1  

UNION

SELECT market_date, total_sales, 'Worst Day' AS sales_day_type
FROM ranked_sales
...
VALUES(24, 'cucumbers','large',8,'unit', CURRENT_TIMESTAMP)
...
SET current_quantity = COALESCE((
			SELECT temp_query.quantity FROM (
					SELECT market_date, product_id, quantity ,
					ROW_NUMBER() OVER (PARTITION BY product_id ORDER BY market_date DESC) last_qty
					from vendor_inventory
			) temp_query
		where temp_query.last_qty = 1 
		AND temp_query.product_id = product_units.product_id), 0)
